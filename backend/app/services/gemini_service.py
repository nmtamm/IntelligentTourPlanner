...
class Visit(BaseModel):
    ...
    expected_time_frame: tuple[datetime.time, datetime.time] = Field(
        description="The expected time frame for the visit."
    )


class Itinerary(BaseModel):
    optional_accommodation: tuple[float, float] = Field(
        description="Accommodation coordinates, optional."
    )
    visits: list[list[Visit]] = Field(
        description="List of visits for the trip, by day."
    )
